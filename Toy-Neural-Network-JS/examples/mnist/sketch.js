...
function testing() {
  ...
  let accuracy = total_correct / total_tests;
  updateLearningRate(accuracy);
  let percent = 100 * (accuracy);
  percent_ele.html(nf(percent, 2, 2) + '%');

  end = Date.now()
  elapsed = end - start
  elapsed = new Date(elapsed)
  elapsed =  elapsed.getMinutes() + ":" + elapsed.getSeconds() + ":" + elapsed.getMilliseconds()
  elapsed_ele.html(nf(elapsed, 2, 2))
  ...
  if (test_index == mnist.test_labels.length) {
    test_index = 0;
    test_count++;
    console.log('Finished Test Set #' + test_count + '');
    console.log(percent);
    console.log(elapsed)
    total_tests = 0;
    total_correct = 0;
...
function updateLearningRate(x){
  if (x > 0.025) {
    let fx = (1 / (9000 * Math.pow(x, 1.2))) + 0.00065;
    nn.setLearningRate(fx)
  }
}
